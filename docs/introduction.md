...
These are the building blocks for your blog routes. For most projects you'll only need `AtomPage` and `Atom`.
...
One operational detail worth knowing upfront: the Atom API is rate-limited to **30 requests per minute per IP**. For most blogs this limit is invisible, but it's worth keeping in mind if you're pre-rendering many posts at build time.
