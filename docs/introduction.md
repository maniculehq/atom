# Build a blog in Next.js with Atom
...
## Set up your blog routes

A complete blog requires just two files. Store your project key in an environment variable (`ATOM_PROJECT_KEY` in `.env.local`) and reference it in each component.
...
**Display a single post** — `AtomPost` fetches and renders one post by its ID. The companion `generatePostMetadata` function pulls title and description from the post so Next.js can set `<head>` tags automatically:
...
|-----------|------|------|-------------|
| `AtomPage` | `projectKey` | `string` | Your project's API key (Bearer token) |
| | `baseRoute` | `string` | Path prefix for post links (e.g. `"/blog"`) |
| `AtomPost` | `projectKey` | `string` | Your project's API key (Bearer token) |
| | `postId` | `string` | The ID of the post to render |

`generatePostMetadata(projectKey, postId)` returns a Next.js `Metadata` object with the post's title and description, suitable for export as `generateMetadata` in a page file.
