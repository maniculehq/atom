...
This page confirms the documentation pipeline is working. If you can read this, docs are building and rendering correctly.
...
## How documentation pages are added
...
2. Add an entry to the `nav` array in `docs/nav.json` with a `title` and `path` (the filename without the extension).
